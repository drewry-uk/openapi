...
function validateDefinitionFileName(fileName) {
    if (!fs.existsSync(fileName))
        exitWithError(`File ${colorette_1.blue(fileName)} does not exist \n`);
...
function iteratePaths(openapi, pathsDir, openapiDir) {
    ...
    if (paths) {
        for (const oasPath of Object.keys(paths)) {
            const pathFile = path.join(pathsDir, ref_utils_1.pathToFilename(oasPath)) + '.yaml';
            ...
            for (const method of types_1.OPENAPI3_METHOD_NAMES) {
                ...
                for (const sample of methodDataXCode) {
                    if (sample.source && sample.source.$ref)
                        continue;
                    const sampleFileName = path.join(openapiDir, 'code_samples', sample.lang, ref_utils_1.pathToFilename(oasPath), method + langToExt(sample.lang));

    ...
    createDescriptionAndExamplePropertyAssigments(node, existingProperties = ts.createNodeArray(), options = {}, sourceFile) {
        ...
        if (!hasExampleOrExamplesKey && examples.length) {
            if (examples.length === 1) {
                const examplePropertyAssignment = ts.createPropertyAssignment('example', ts.createLiteral(examples[0]));
                propertyAssignments.push(examplePropertyAssignment);
            }
            else {
                const examplesPropertyAssignment = ts.createPropertyAssignment('examples', ts.createArrayLiteral(examples.map((e) => ts.createLiteral(e))));
